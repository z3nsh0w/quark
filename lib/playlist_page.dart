          ...
          child: Material(
            ...
            child: Stack(
              children: [
                // Positioned.fill(
                //   child: FutureBuilder(
                //     future: ImageCacheService().getImage(nowPlayingTrack.cover),
          ...
          child: Stack(
            children: [
              Positioned.fill(
                child: AnimatedSwitcher(
                  duration: Duration(
                    milliseconds: (550 * transitionSpeed).round(),
                  ),
                  transitionBuilder:
                      (Widget child, Animation<double> animation) {
                        return FadeTransition(opacity: animation, child: child);
                          ...
                          builder: (context, snapshot) {
                            if (snapshot.data != null) {
                              return TrackCoverImage(
                                ...
                                track: nowPlayingTrack,
                                fit: BoxFit.cover,
                                applyDarkenFilter: true,
                              );
                            } else {
                              return Container(
                                color: Color.fromRGBO(24, 24, 26, 1),
                              );
                            }
                          },
                        ),
                              ...
                              child: AnimatedExpand(
                                ...
                                content: Row(
                                  ...
                                  children: [
                                      ...
                                      Row(
                                        children: [
                                          animatedExpandButton(() async {
                                            double speed =
                                                player.player.playbackRate;
                                            print(speed);
                                            await player.player.setPlaybackRate(
                                              speed -= 0.2,
                                            );
                                            setState(() {});
                                          }, Symbols.speed_0_75),
                                    ...
                                    animatedExpandButton(() async {
                                      final bool?
                                      recursiveFilesAdding = await Database.get(
              ...
              if (playlistOpeningArea && !settingsView)
                Positioned(
                  left: 0,
                  child: MouseRegion(
                    onEnter: (event) {
...
class TrackCoverImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (track is LocalTrack) {
    ...
    } else if (track.cover.isNotEmpty) {
      image = CachedImage(
      ...
      image = Container(
        width: width,
        height: height,
        color: const Color.fromRGBO(24, 24, 26, 1),
      );
    }

    if (applyDarkenFilter) {
